...
class FollowerOpinionLeaderDataReader(DataReader):
    ...
    def __init__(self, resultsFolderStructure, resultsDir):
        ...
        self._metricScoreECDFMap = dict()
        self._metricScewValues = dict()
    ...
    def registerMetric(self, metric):
        ...
        metricFileName = self._resultsDir + "/" + "follower" + "_" + metric.name() + ".txt"
        f = open(metricFileName, "w")
        self._metric_files[metric.name()] = f
        opinionLeaderFileName = self._resultsDir + "/" + "follower" + "_" + metric.name() + "_ol.txt"
    ...
    def readTurn(self, turn):
        followerData = turn.get(FollowerData.FOLLOWER_DATA_KEY)
        schoolClass = turn.parent()
        
        if followerData != None:
            print(followerData.csvNetworkData._csvfile)
            resultFolder = self._resultsFolderStructure.turnFolderPath(turn)
            for metric in self._metrics:
                result = metric.compute(followerData._network)
                key = metric.unique_name()
                turn.add(key, result)
                result.writeToFile(resultFolder, "follower")
    ...
    def read(self, schools):
        ...
        monotonicy_stat_file_path = self._resultsFolderStructure.rootDir() + "follower_monotonicy.txt";
        monotonicy_stat_file = open(monotonicy_stat_file_path, "w")
        
        opinion_leader_number_stat_file_path = self._resultsFolderStructure.rootDir() + "follower_ol_nr_stat.txt";
